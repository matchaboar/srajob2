...
async def test_existing_url_is_skipped_via_seen_urls_dedup(monkeypatch: pytest.MonkeyPatch):
    ...
    async def fake_fetch_seen_urls_for_site(
...
@pytest.mark.asyncio
async def test_convex_prod_seen_url_query_returns_existing_url():
    """
    Integration test: verify that listSeenJobUrlsForSite correctly returns
    the target URL when queried from Convex prod.

    This test calls the REAL Convex prod endpoint to verify dedup state.
    If this test fails, it means the URL is NOT in seen_job_urls table,
    which would explain why it gets re-scraped.

    Run with: uv run pytest -k "convex_prod_seen" -v
    """
    import os

    convex_url = os.getenv("CONVEX_URL") or os.getenv("CONVEX_HTTP_URL")
    if not convex_url:
        pytest.skip("Skipping integration test - CONVEX_URL/CONVEX_HTTP_URL not set")

    from job_scrape_application.workflows.helpers.scrape_utils import fetch_seen_urls_for_site

    # Query Convex to see if TARGET_URL is recorded as seen for Airbnb
    seen_urls = await fetch_seen_urls_for_site(
        source_url=AIRBNB_SOURCE_URL,
        pattern=None,
        candidate_urls=[TARGET_URL],
    )

    # If this assertion fails, the URL is NOT in seen_job_urls for this source
    assert TARGET_URL in seen_urls, (
        f"DEDUP BUG: {TARGET_URL} is NOT recorded in seen_job_urls for source={AIRBNB_SOURCE_URL}. "
        f"This means the URL will be re-scraped on next run! "
        f"Seen URLs returned: {seen_urls}"
    )


@pytest.mark.asyncio
async def test_convex_prod_job_url_keys_contains_url():
    """
    Integration test: verify that findExistingJobUrls correctly identifies
    the target URL as existing in the global job_url_keys table.

    This is the second layer of dedup - even if seen_job_urls is empty,
    this should catch duplicates.

    Run with: uv run pytest -k "convex_prod_job_url" -v
    """
    import os
    ...
    existing_urls = await acts.filter_existing_job_urls([TARGET_URL])

    # If this assertion fails, the URL is NOT in job_url_keys table
    assert TARGET_URL in existing_urls, (
        f"DEDUP BUG: {TARGET_URL} is NOT in job_url_keys table. "
        f"This means even if seen_job_urls fails, the global dedup won't catch it. "
        f"Existing URLs returned: {existing_urls}"
    )


@pytest.mark.asyncio
async def test_convex_prod_job_url_mismatch():
    """
    Integration test: verify that the job exists and check its stored URL.

    The job exists with ID k179yqdb18vztc0hs6kt55bmv57yyfkz but getJobIdByUrl
    didn't find it. This test investigates the URL mismatch.

    Run with: uv run pytest -k "convex_prod_job_url_mismatch" -v -s
    """
    import os
    ...
    from job_scrape_application.services.convex_client import convex_query

    # Get the job by its known ID
    job = await convex_query("jobs:getJobById", {"id": CONVEX_JOB_ID})

    assert job is not None, f"Job {CONVEX_JOB_ID} should exist in Convex prod"

    stored_url = job.get("url")
    print(f"\n=== URL MISMATCH ANALYSIS ===")
    print(f"Job ID: {CONVEX_JOB_ID}")
    print(f"Stored URL: {stored_url}")
    print(f"Target URL: {TARGET_URL}")
    print(f"URLs match: {stored_url == TARGET_URL}")

    # Check if stored URL is in dedup tables
    if stored_url:
        seen_urls = await convex_query(
            "router:listSeenJobUrlsForSite",
            {"sourceUrl": AIRBNB_SOURCE_URL, "urls": [stored_url]}
        )
        print(f"Stored URL in seen_job_urls: {stored_url in seen_urls.get('urls', [])}")

        existing = await convex_query("router:findExistingJobUrls", {"urls": [stored_url]})
        print(f"Stored URL in job_url_keys: {stored_url in existing.get('existing', [])}")

    # This assertion will fail to show the mismatch
    assert stored_url == TARGET_URL, (
        f"URL MISMATCH BUG: Job stored with URL '{stored_url}' but we're checking '{TARGET_URL}'. "
        f"This causes dedup to fail because URLs don't match exactly."
    )
