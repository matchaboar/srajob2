...
async def test_verkada_greenhouse_job_detail_fixture_normalizes_job():
    ...
    # The extractor now prefers updated_at over first_published for Greenhouse jobs
    updated_at_value = job_payload.get("updated_at") or job_payload.get("first_published")
    expected_posted_at, expected_unknown = parse_posted_at_with_unknown(updated_at_value)
