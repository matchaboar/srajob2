...
def test_process_job_batch_now_filters_existing_urls_before_spidercloud(monkeypatch):
    """
    ...
    asyncio.run(activities.process_spidercloud_job_batch(batch, persist_scrapes=True))
...
def test_all_urls_skipped_when_all_exist(monkeypatch):
    """
    ...
    result = asyncio.run(activities.process_spidercloud_job_batch(batch, persist_scrapes=True))
...
def test_filter_new_job_urls_is_more_efficient(monkeypatch):
    """
    ...
    def fake_convex_query(endpoint: str, args: dict) -> dict:
    ...
    # Patch the convex_query function at the source module
    monkeypatch.setattr(convex_client_module, "convex_query", fake_convex_query)

    # Test OLD approach: filter_existing_job_urls (returns 95 URLs)
    existing = activities.filter_existing_job_urls(all_urls)
    new_urls_via_filter = [u for u in all_urls if u not in existing]

    # Test NEW approach: filter_new_job_urls (returns 5 URLs)
    new_urls_direct = activities.filter_new_job_urls(all_urls)
...
def test_filter_new_job_urls_error_raises_for_caller_fallback(monkeypatch):
    """
    ...
    def failing_convex_query(endpoint: str, args: dict) -> dict:
        """Mock Convex query that always fails."""
        raise RuntimeError("Simulated Convex failure")

    monkeypatch.setattr(convex_client_module, "convex_query", failing_convex_query)
    ...
    with pytest.raises(RuntimeError, match="Simulated Convex failure"):
        activities.filter_new_job_urls(test_urls)
...
def test_record_scrape_url_attempts_receives_only_expected_fields(monkeypatch):
    """
    ...
    def capturing_record_scrape_url_attempts(entries: List[Dict[str, Any]]) -> None:
    ...
    print("   - Input rows had fields: _id, url, sourceUrl, provider, pattern, postedAt, siteId, urlType, attempts")
    print(f"   - Entries passed to Convex have only: {ALLOWED_FIELDS}")
    print("   - Extra fields correctly filtered out")
