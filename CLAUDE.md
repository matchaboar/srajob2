...
- After editing files with Convex/httpx calls: run `uv run scripts/lint_dbos_step.py`
...
uvx ruff check .                                # Lint
uv run scripts/lint_dbos_step.py                # Check @DBOS.step decorator
...
- **DBOS step rule**: Functions calling `convex_query`, `convex_mutation`, `convex_action`, or `httpx` methods must be decorated with `@DBOS.step`. Use `# noqa: DBOS001` to exempt a function.
- **Convex sync rule**: Functions calling convex functions (directly or transitively) must not be async and must not await convex calls. The Python convex package is synchronous.
- **Async workflow rules**: Coroutine (async) workflows must:
  - Use `DBOS.start_workflow_async` instead of `DBOS.start_workflow` (DBOS005)
  - Use `enqueue_async` instead of `enqueue` (DBOS005)
  - Use `DBOS.sleep_async`, `DBOS.recv_async`, `DBOS.send_async`, `DBOS.set_event_async`, `DBOS.get_event_async` instead of their sync counterparts (DBOS006)
- **No raw sleep**: DBOS workflows and steps must not use `time.sleep` or `asyncio.sleep` (DBOS007). Use `DBOS.sleep` or `DBOS.sleep_async` instead for durable sleep that survives workflow recovery.
