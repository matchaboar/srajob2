...
def test_lease_scrape_url_batch_filters_skip_and_marks_failed(monkeypatch):
    """Note: This function is now sync (was async) as part of the DBOS refactoring."""
    leased = {
        "urls": [
            {
    ...
    def fake_fetch_seen(source_url: str, pattern: str | None):
        assert source_url == "https://example.com"
        return ["https://example.com/skip-me"]

    monkeypatch.setattr(
        "job_scrape_application.dbos_runtime.queue.lease_scrape_url_batch",
    ...
    monkeypatch.setattr(
        "job_scrape_application.workflows.activities._archive.queue_management.fetch_seen_urls_for_site",
        fake_fetch_seen,
    )

    res = acts.lease_scrape_url_batch("spidercloud", 5)
...
def test_lease_scrape_url_batch_keeps_listing_urls_out_of_seen(monkeypatch):
    """Note: This function is now sync (was async) as part of the DBOS refactoring."""
...
def test_lease_scrape_url_batch_handles_non_dict_response(monkeypatch):
    """Note: This function is now sync (was async) as part of the DBOS refactoring."""
    def fake_lease_scrape_url_batch(**_kwargs):
        return None
    ...
    res = acts.lease_scrape_url_batch("spidercloud", 2)

    assert res == {"urls": [], "skippedUrls": []}


def test_lease_scrape_url_batch_retries_when_all_skipped(monkeypatch):
    """Note: This function is now sync (was async) as part of the DBOS refactoring."""
    lease_payloads = [
        {
            "urls": [
    ...
    res = acts.lease_scrape_url_batch("spidercloud", 1)
