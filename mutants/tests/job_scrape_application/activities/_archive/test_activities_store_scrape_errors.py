...
def test_store_scrape_omits_null_cost(monkeypatch):
    payload = {
...
def test_store_scrape_omits_null_pattern(monkeypatch):
    payload = {
        "sourceUrl": "https://example.com",
        "pattern": None,
        "items": {"normalized": []},
...
def test_store_scrape_marks_spidercloud_captcha_failed(monkeypatch):
    payload = {
...
def test_store_scrape_all_jobs_skipped_logs_debug(monkeypatch):
    payload = {
    ...
    def fake_filter_new(urls: list[str]):
        # Return empty list to indicate all URLs already exist (none are new)
        return []
    ...
    monkeypatch.setattr(acts, "trim_scrape_for_convex", lambda x, **kwargs: x)
    monkeypatch.setattr(acts, "filter_new_job_urls", fake_filter_new)
    monkeypatch.setattr("job_scrape_application.services.convex_client.convex_mutation", fake_mutation)
...
def test_store_scrape_retries_on_failure(monkeypatch):
    payload = {
        "sourceUrl": "https://example.com",
        "items": {"normalized": []},
        "costMilliCents": 1500,
...
def test_store_scrape_ingest_jobs_failure_is_nonfatal(monkeypatch):
...
def test_store_scrape_raises_after_double_failure(monkeypatch, caplog):
    payload = {
        "sourceUrl": "https://example.com",
        "items": {"normalized": [{"url": "https://example.com/job"}]},
    }
...
def test_store_scrape_payload_matches_router_contract(monkeypatch):
...
def test_store_scrape_applies_heuristics_before_ingest(monkeypatch):
    payload = {
        "sourceUrl": "https://example.com",
        "items": {
            "normalized": [
...
def test_store_scrape_heuristic_cancellation_logs_and_continues(monkeypatch):
    payload = {
        "sourceUrl": "https://example.com",
        "workflowName": "SpidercloudJobDetails",
        "workflowId": "wf-123",
