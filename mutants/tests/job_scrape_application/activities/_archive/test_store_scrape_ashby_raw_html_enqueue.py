...
def test_store_scrape_enqueues_ashby_job_urls_only(monkeypatch):
    payload = _load_api_payload()
...
def test_store_scrape_enqueues_ashby_urls_from_raw_html(monkeypatch):
    html = _load_html()
    scrape = _build_scrape(html)
...
def test_store_scrape_enqueues_ashby_ramp_urls_from_api(monkeypatch):
    payload = _load_ramp_api_payload()
    scrape = {
        "sourceUrl": "https://jobs.ashbyhq.com/ramp",
        "startedAt": 1,
