...
from job_scrape_application.workflows._archive import temporal_greenhouse_workflow as gh  # noqa: E402
...
async def test_greenhouse_workflow_uses_args_kw(monkeypatch):
    ...
    def fake_execute_activity(activity, *args, **kwargs):
        ...
        if activity is gh.filter_existing_job_urls:
            return []
        if activity is gh.compute_urls_to_scrape:
            return {
                "urlsToScrape": ["https://example.com/job/1"],
                "existingCount": 0,
                "totalCount": 1,
            }
