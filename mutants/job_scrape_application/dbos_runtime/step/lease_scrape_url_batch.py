"""DBOS step function for leasing scrape URL batches from SQLite queue."""
...
from ..queue import lease_scrape_url_batch as _lease_from_queue, complete_scrape_urls
from ...workflows.helpers.step import fetch_seen_urls_for_site
from ...workflows.site_handlers import get_site_handler

logger = logging.getLogger("dbos.step.queue")
...
def lease_scrape_url_batch_step(
    provider: Optional[str] = None,
    limit: int = SPIDERCLOUD_BATCH_SIZE,
    url_type: str | None = None,
) -> Dict[str, Any]:
    """Lease a batch of queued URLs from the SQLite queue.

    This step leases URLs from the SQLite queue, filters out already-seen URLs,
    and returns the batch ready for scraping.

    Args:
        provider: Optional provider filter (e.g., "spidercloud").
        limit: Maximum number of URLs to lease.
        url_type: Optional URL type filter ("listing" or "detail").

    Returns:
        Dict with "urls" list and "skippedUrls" list.
    """
    ...
    while attempts < max_attempts:
        attempts += 1
        lease = _lease_from_queue(provider=provider, limit=limit, url_type=url_type)
...
@DBOS.step(retries_allowed=True, max_attempts=3, interval_seconds=0.5, backoff_rate=2.0)
def fail_listing_batch_urls_step(
    batch: Dict[str, Any],
    error: str = "batch_failed",
) -> Dict[str, Any]:
    """Mark a leased listing batch as failed.

    Args:
        batch: The batch dict containing "urls" list.
        error: The error reason to record.
