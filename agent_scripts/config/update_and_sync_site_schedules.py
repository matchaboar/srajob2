...
async def _push_to_convex(entries: List[Dict[str, Any]], *, skip_sync: bool = False) -> None:
    ...
    site_names = [entry.get("name") for entry in entries if isinstance(entry, dict) and entry.get("name")]
    enabled_sites = [entry.get("name") for entry in entries if isinstance(entry, dict) and entry.get("name") and entry.get("enabled", True)]
    disabled_sites = [entry.get("name") for entry in entries if isinstance(entry, dict) and entry.get("name") and not entry.get("enabled", True)]

    print(f"\nSyncing {len(site_names)} sites to Convex:")
    print(f"  Enabled ({len(enabled_sites)}): {', '.join(sorted(enabled_sites))}")
    if disabled_sites:
        print(f"  Disabled ({len(disabled_sites)}): {', '.join(sorted(disabled_sites))}")
    print()
