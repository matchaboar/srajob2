...
async def _run_scrape_workflow(
    ...
    *,
    ...
    max_concurrency = max(1, runtime_config.temporal_general_worker_count)

    async def _process_site(site: dict[str, Any]) -> dict[str, Any]:
        local_scrape_ids: list[str] = []
        try:
            workflow_context = {
            ...
            if persist_scrapes and scrape_id:
                local_scrape_ids.append(scrape_id)
            else:
                if isinstance(res, dict):
            ...
            return {"scrape_ids": local_scrape_ids, "status": "completed", "failure_reason": None}
            ...
            return {
                "scrape_ids": local_scrape_ids,
                "status": "failed",
                "failure_reason": f"{site.get('url')}: {reason}",
            }
    ...
    try:
        ...
        while True:
            if max_leases is not None and leased_count >= max_leases:
                break
            lease_slots = max_concurrency
            if max_leases is not None:
                lease_slots = min(lease_slots, max_leases - leased_count)
            if lease_slots <= 0:
                break

            leased_sites: list[dict[str, Any]] = []
            for _ in range(lease_slots):
            ...
            results = await asyncio.gather(
                *[_process_site(site) for site in leased_sites],
            )
            for result in results:
                scrape_ids.extend(result.get("scrape_ids", []))
...
class SpidercloudJobDetailsWorkflow:
    ...
    async def run(self) -> ScrapeSummary:  # type: ignore[override]
        ...
        try:
            while True:
                if workflow.now() - run_started > max_run_duration:
                    break
                await workflow_checkpoint(
                ...
                try:
                    ...
                    if persist_scrapes:
                        ...
                        if scrapes:
                            ...
                            max_store_concurrency = max(1, runtime_config.spidercloud_job_details_concurrency)
                            semaphore = asyncio.Semaphore(max_store_concurrency)

                            async def _store_scrape(scrape: Dict[str, Any]) -> tuple[str, str | None, str | None]:
                            ...
                            for idx, scrape in enumerate(scrapes):
                                if not isinstance(scrape, dict):
                                    continue
                                tasks.append(asyncio.create_task(_store_scrape(scrape)))
                                await _yield_if_needed(idx)

                            results = await asyncio.gather(*tasks) if tasks else []
                            for status_val, url_val, res_id in results:
