"""DBOS step function for scraping listing URLs via SpiderCloud."""
...
async def scrape_listing_urls(
    ...
    """Scrape listing URLs via SpiderCloud API.

    This step calls the SpiderCloud API to scrape listing pages and extract
    job URLs. It handles retries automatically via DBOS.

    Args:
        urls: List of listing page URLs to scrape
    ...
    Returns:
        Dict containing the scrape response with extracted items.
        Structure: {"scrape": {...}, "jobsScraped": int} or similar

    Raises:
        Exception: On non-retryable errors after retry attempts exhausted.
    """
    if not urls:
        return {"scrape": None, "jobsScraped": 0, "urls": []}
    ...
    return result or {"scrape": None, "jobsScraped": 0, "urls": urls}
