...
# Import from refactored modules (for re-export and to allow gradual migration)
...
async def process_spidercloud_job_batch(
    ...
    for row in batch.get("urls", []):
        ...
        if not isinstance(url_val, str) or not url_val.strip():
            continue
        url_type = row.get("urlType") if isinstance(row.get("urlType"), str) else None
